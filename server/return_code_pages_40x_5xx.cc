...
void	change_default_html_return_code(std::string path, std::string *return_code)
{
	...
	else
	{
		...
		if (file)
		{
			...
			throw std::invalid_argument("Ifstream failed to open file " + it->first);
		return_code.clear();
...
void	set_default_return_code(T & datas)
{
	...
	it = (Location *)(data)->get_error_pages().begin();
	it_end = (Location *)(data)->get_error_pages().end();
	return_codes = &((Location *)(data))->return_codes;
	initialize_html_return_code_page(&(*return_codes));
	root_path = data->get_root_path();
	for (; it != it_end; ++it)
	{
		switch (*it->first)
		{
